...
cross-coupled PMOS/NMOS regeneration, NOR-based SR latch to hold the
decision through precharge. Single-phase clock, no inverters.
...
transient). gear + reltol=1e-6 + per-DUT decks give clean sign behavior
down to the 10 uV point (~3x the 3.3 V * 1e-6 solver floor).
...
* SR latch, cross-coupled NOR2 (on1=on2=0 during precharge -> hold)
* INP > INM -> on2 rises -> Q high
XQP1 nq1 on1 VDD VDD {PF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XQP2 Q   QB  nq1 VDD {PF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XQN1 Q   on1 VSS VSS {NF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XQN2 Q   QB  VSS VSS {NF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XPP1 nq2 on2 VDD VDD {PF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XPP2 QB  Q   nq2 VDD {PF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XPN1 QB  on2 VSS VSS {NF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
XPN2 QB  Q   VSS VSS {NF} W={WUNIT} L={p['L']} nf=1 m={m(p['W_LG'])}
.ends
"""
...
.lib {PDK_LIB} tt
{comp_subckt(p)}
.options method=gear reltol=1e-6 vntol=1e-9 abstol=1e-15
VDDS vdd 0 {VDD}
...
def deck_point(p, cm, dv, tag):
    return deck_head(p) + f"""
...
def measure(p):
    ...
    for i, cm in enumerate(CMS):
        for j, dv in enumerate(DVS):
            ...
            open(f"spice/comp_{tag}.spice", "w").write(
                deck_point(p, cm, dv, tag))
            jobs.append(f"comp_{tag}.spice")
    open("spice/comp_kick.spice", "w").write(deck_kick(p))
    ...
    json.dump(dict(tau_ps=round(tau * 1e12),
                   ...
                   tdec_ns={f"{cm}/{dv:.0e}": round(r["td"] * 1e9, 3)
                            ...
                            if np.isfinite(r["td"])}),
              open("reports/results/comp.json", "w"), indent=1)
    if fail:
        sys.exit(1)
